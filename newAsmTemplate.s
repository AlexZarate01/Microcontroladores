...
; 5. Create a variable on Access RAM called AUX and load it with literal 0xE6.
    ;Then, shift its value one bit to the right using RRNCF (e.g., 11100110 ? 01110011).
    MOVLW   11100110B  ;Move 0xE6 to WREG
    MOVWF   BANKMASK(AUX),0 ;Move WREG to masked AUX address (Access Bank)
    RRNCF   BANKMASK(AUX),0,0 ;shift its value one bit to the right using RRNCF (Rotate Right f (No Carry))
    
; 6. Load literal 0x67 to WREG and shift it two bits to the left using RLNCF (Rotate Left f (No Carry).
    MOVLW 0x67
    RLNCF WREG,0,0
    RLNCF WREG,0,0
   ;10011101
        
; 7. Load literal 0xB7 to a variable called TST1 and invert only *even bits. Bit order goes from 0 to 7 and are counted from right to left (e.g., 10110111 ? 11100010). Load the result on WREG.
    MOVLW   0xB7
...
;8. Load literal 0x54 to a variable called TST2 and set to 1 only *odd bits. Bit order goes from 0 to 7 and are counted from right to left (e.g., 01010100 ? 11111110). Load the result on WREG.
    MOVLW   0x54
...
LOOP:
  MOVLW   PORTB,0,0	;Move PORTB to WREG
